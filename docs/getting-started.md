...
**Settings → Notifications → add channel.** Slack and Discord are one click if your operator has connected them; otherwise paste an incoming webhook URL. Email, SMS, PagerDuty, Telegram, Mattermost, ntfy, Pushover, Gotify, and plain webhooks are all there too.
