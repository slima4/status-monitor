...
- **Sticky last-good for domain-expiry probes.** Each successful RDAP probe writes `(expiry_at, registrar, verified_at)` to `domain_expiry_state` (FK CASCADE on the target). A subsequent transient failure — RDAP timeout, throttle drop, registry 5xx, 404 — does not flip the monitor: the executor reads the cached row and emits a `CheckResult` with the cached verdict + an `error="served_stale: …"` annotation, so the customer's status page keeps its real expiry signal. Cached rows older than 7d escalate to `Error`, which is alert-eligible. Cross-tenant singleflight (keyed by canonical domain) collapses concurrent probes for the same domain to one outbound request — RDAP is public registry data, coalescing across tenants is safe and IANA-friendly.
