...
The `[retention]` section sets the long-horizon windows. Defaults: `login_attempts_days = 180`, `quota_events_days = 90`, `audit_log_days = 730`. Check-result retention is **not** a config knob — it is a single flat 90-day `TTL` baked into the ClickHouse table at migration time (a value here would be silently ignored, since the TTL is never re-issued as an `ALTER` on boot). The 90 days is the one authority: the public status page's daily history strip and the Privacy Policy both pin the same number. Session idle/absolute reaping uses `[auth.session]`; soft-deleted org/user grace uses `tenancy.deletion_grace_period_days`; OAuth-state and magic-link tokens are swept by their own short-cadence jobs.
