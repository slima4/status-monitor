...
- **Destination deny-list**: the customer-controlled outbound URL (`slack`/`discord`/`msteams`/`google_chat`/`webhook`/`ntfy`'s and `gotify`'s `server_url`) is checked against the platform's abuse deny-list on create, update, and both test endpoints — a match is rejected (`ABUSE_BLOCKED` / `DOMAIN_DENYLISTED`). `telegram`/`whatsapp`/`email`/`pagerduty`/`pushover`/`sms` deliver to fixed vendor endpoints.
