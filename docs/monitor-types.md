...
- A response body is read up to 1 MiB. A page bigger than that still passes on the status you expect, and its recorded size is blank because the read stopped early. Only a body substring assertion needs the whole page, and when it cannot be evaluated the result says `body over the 1 MiB read cap` rather than reporting a transport failure. Large homepages are one more reason to assert against a health endpoint.
