...
/// Personal-org create on signup. Bypasses [`create_org_with_owner`]'s
/// per-user owner-limit because the very first org for a new user is what
/// makes their account usable — the limit only kicks in for user-named orgs
/// after that.
///
/// The slug is generated by [`generate_personal_slug`]; on the rare collision
...
pub async fn create_personal_org_with_owner_in_tx(
    ...
    .fetch_optional(&mut **tx)
    .await
    .context("create_personal_org_with_owner_in_tx: insert org")?;
    ...
    sqlx::query("INSERT INTO memberships (user_id, org_id, role) VALUES ($1, $2, 'owner')")
        ...
        .execute(&mut **tx)
        .await
        .context("create_personal_org_with_owner_in_tx: insert membership")?;
    record_audit_tx(
        &mut *tx,
        OrgId(org_id),
        Some(user),
    ...
    .await
    .context("create_personal_org_with_owner_in_tx: audit")?;
